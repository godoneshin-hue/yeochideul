...
import { SURVEY_DIET, SURVEY_SLEEP, SKIN_TYPES, ACNE_CONCERNS, STRESS_LEVEL, WATER_INTAKE, EXERCISE_FREQ, MAKEUP_FREQ } from "@/lib/yeochi-data";
...
export function SurveyScreen() {
  ...
  const submit = () => {
    ...
    setUser((u) => ({ ...u, age, gender, height, weight, skinType, concern, diet, sleep, stress, water, exercise, makeup, bmi, surveyDone: true }));
    setCelebrate(true);
  ...
  return (
    <div className="min-h-full px-6 py-6 animate-fade-in-up relative">
      ...
      <Card title="라이프스타일" onTap={pop}>
        ...
        <Choice label="운동 빈도" options={EXERCISE_FREQ} value={exercise} onChange={setExercise} small />
      ...
      <div className="fixed inset-0 pointer-events-none">
        {Array.from({ length: heart }).slice(-3).map((_, i) => (
          <Heart key={heart - i} className="absolute text-primary fill-primary animate-heart-pop"
...
function Card({ title, children, onTap }: { title: string; children: React.ReactNode; onTap?: () => void }) {
  return (
    <div onClick={onTap} className="bg-card rounded-3xl p-5 mb-4 shadow-card border border-border/50 space-y-3">
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onChange(Math.max(min, value - 1))} className="w-8 h-8 rounded-full bg-secondary active:scale-90 transition font-bold">−</button>
        <span className="font-bold w-12 text-center">{value}</span>
        <button onClick={() => onChange(Math.min(max, value + 1))} className="w-8 h-8 rounded-full bg-primary text-primary-foreground active:scale-90 transition font-bold">+</button>
      </div>
    </div>
  );
}

function Choice({ label, options, value, onChange, small }: { label: string; options: readonly string[] | string[]; value: string; onChange: (v: string) => void; small?: boolean }) {
  return (
    <div>
