      ...
      <div className="p-5 animate-fade-in-up">
        <div className="bg-card rounded-2xl p-4 shadow-card border border-border/50 space-y-2 mb-4">
          ...
          <button
            onClick={add}
            disabled={saving}
            className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm active:scale-95 transition flex items-center justify-center gap-1 disabled:opacity-60"
          >
            <Plus className="w-4 h-4" /> {saving ? "등록 중..." : "등록"}
          </button>
        </div>
          ...
          <div className="space-y-2">
            {user.cosmetics.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="bg-card rounded-2xl p-4 shadow-card border border-border/50 flex items-center gap-3 animate-fade-in-up"
                >
                  <div
                    className={`w-1.5 h-12 rounded-full ${over ? "bg-destructive" : warn ? "bg-amber-400" : "bg-emerald-400"}`}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm truncate">{c.name}</div>
                    <div className="text-[11px] text-muted-foreground">
                      개봉 {c.openDate} · 만료 {exp}
                    </div>
                    <div
                      className={`text-xs font-semibold mt-0.5 ${over ? "text-destructive" : warn ? "text-amber-600" : "text-emerald-600"}`}
                    >
                  ...
                  <button
                    onClick={() => del(c.id)}
                    className="p-2 rounded-full hover:bg-destructive/10 text-destructive active:scale-90 transition"
                  >
